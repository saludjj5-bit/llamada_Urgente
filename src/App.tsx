...
export default function App() {
  ...
  // AUTO-RECONEXIÓN FURIOZA (SEGUNDO PLANO)
  useEffect(() => {
    const forceReconnect = () => {
      if (document.visibilityState === 'visible' && !isConnected && userGroupId) {
        connect();
      }
    };
  ...
  }, [isConnected, userGroupId, connect]);
  ...
  useEffect(() => {
    if (userRole) {
      ...
      const unsubUsers = onSnapshot(usersRef, (snap) => {
        ...
        if (userGroupId) {
          const talker = usersData.find(u => u.groupId === userGroupId && u.isTalking === true && u.uid !== user?.uid);
  ...
  // REPARACIÓN DEL BOTÓN INFERNAL (INICIO)
  ...
  // REPARACIÓN DEL BOTÓN INFERNAL (SOLTAR)
  const handlePressEnd = async (e?: React.MouseEvent | React.TouchEvent) => {
    if (e && e.cancelable) e.preventDefault();
    // QUITADO EL BLOQUEO QUE IMPEDÍA CORTAR EL MICRÓFONO
    setIsPressed(false);
    if (user) {
      try { await updateDoc(doc(db, 'users', user.uid), { isTalking: false }); } catch (err) {}
    }
    stopTalking(); // ORDEN ABSOLUTA DE APAGAR EL MICRÓFONO
  };
    ...
    return (
      <div className="min-h-screen bg-white text-gray-900 flex flex-col items-center justify-center p-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 text-center space-y-8 max-w-sm w-full">
          ...
          {authError && (<div className="p-4 bg-red-500/5 border border-red-500/10 rounded-2xl flex items-center gap-3 text-red-600 text-sm text-left"><ShieldAlert className="w-5 h-5 flex-shrink-0" /><p>{authError}</p></div>)}
          <div className="space-y-3 text-left">
            <input type="email" placeholder="Correo electrónico" value={emailInput} onChange={(e) => setEmailInput(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 outline-none focus:ring-2 focus:ring-blue-500" />
            <input type="password" placeholder="Contraseña" value={passInput} onChange={(e) => setPassInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSignIn()} className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 outline-none focus:ring-2 focus:ring-blue-500" />
            <button onClick={handleSignIn} className="w-full py-4 mt-2 bg-blue-600 text-white font-bold rounded-2xl flex items-center justify-center gap-3 hover:bg-blue-700 shadow-lg"><LogIn className="w-5 h-5" /> Entrar</button>
            {authError && (<button onClick={() => { setAuthError(null); setEmailInput(''); setPassInput(''); logOut(); setIsAuthLoading(false); }} className="w-full py-2 text-gray-400 hover:text-gray-600 text-xs font-bold uppercase tracking-widest text-center">Limpiar Error y Cancelar</button>)}
          </div>
        </motion.div>
      </div>
    );
    ...
    <div className="min-h-screen bg-white text-gray-900 font-sans selection:bg-blue-500/30 flex flex-col items-center justify-center p-4 overflow-hidden">
        ...
        <div className="flex items-center gap-3 bg-white/80 border border-gray-200 px-3 py-2 rounded-2xl shadow-sm backdrop-blur-md">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-1 mt-1">
              ...
              {userGroupName && (<><span className="text-[8px] text-gray-200">•</span><span className="text-[8px] font-bold text-blue-600 uppercase tracking-widest">{userGroupName}</span></>)}
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {userRole === UserRole.ADMIN && (<button onClick={() => setIsMonitorOpen(!isMonitorOpen)} className={cn("p-3 rounded-2xl border transition-all", isMonitorOpen ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-400")}><Radio className="w-5 h-5" /></button>)}
          ...
          <button onClick={logOut} className="p-3 rounded-2xl bg-white border border-gray-200 text-gray-400 hover:text-red-500"><LogOut className="w-5 h-5" /></button>
        </div>
      </div>
      ...
      <div className="relative w-full max-w-md flex flex-col items-center gap-8 mt-16">
        <div className="text-center space-y-2">
          ...
          <motion.div className="inline-flex items-center gap-3 px-6 py-2.5 rounded-2xl bg-red-600 text-white mx-auto">
        ...
        <div className="relative w-72 h-[450px] bg-gray-50 rounded-[60px] border-[8px] border-gray-100 shadow-2xl flex flex-col items-center p-6">
          <div className="w-full h-32 bg-white rounded-3xl border border-gray-100 p-4 shadow-inner flex flex-col items-center justify-center gap-2 mt-8">
            ...
            <div className="w-full text-center">
              {!userGroupId ? (<p className="text-[10px] text-red-500 font-black uppercase animate-pulse">Sin Grupo Asignado</p>) : isTalking ? (<div className="flex flex-col items-center gap-1"><span className="text-[8px] font-black uppercase text-red-600 animate-pulse">Transmitiendo</span></div>) : talkingUser ? (<div className="flex flex-col items-center gap-1"><span className="text-[8px] font-black uppercase text-blue-600 animate-pulse">Recibiendo</span><p className="text-[10px] font-black">{talkingUser.displayName || talkingUser.email}</p></div>) : (<p className="text-[10px] text-gray-400 italic">Esperando señal...</p>)}
            </div>
          </div>

          <div className="flex-1 flex flex-col items-center justify-center w-full gap-8">
            {/* NUEVOS EVENTOS TÁCTILES ANTI-ATASCO */}
            <motion.button
              onMouseDown={handlePressStart} onMouseUp={handlePressEnd} onMouseLeave={handlePressEnd}
              onTouchStart={handlePressStart} onTouchEnd={handlePressEnd} onTouchCancel={handlePressEnd}
              disabled={!isConnected}
              animate={{ scale: isPressed ? 0.9 : 1, backgroundColor: isPressed ? "#3b82f6" : isConnected ? "#2563eb" : "#3f3f46" }}
              className="w-32 h-32 rounded-full flex items-center justify-center shadow-2xl origin-center"
            >
